...
public class ModeInputPanel extends GridPane {
    ...
    private int getFiles(ListView<String> listView) { // TODO
    	...
    	File board = new File("Boards");
    	File[] files = board.listFiles();
    	int ret = 0;
    	for (int i = 0; i < files.length; i++) {
    		
    		if (files[i].getName() == "Starter.txt") {
    			ret = i;
    		}
    ...
    private void selectBoard(Label selectBoardLabel, ListView<String> boardsList) { // TODO
    	...
    	String selected = boardsList.getSelectionModel().getSelectedItem();
    	selectBoardLabel.setText(selected);
