...
public class View {
    ...
    protected void runMove() { // TODO
    	...
    	if(this.model.isHumansPlaying()) {
	    	...
	    	if (board.getTurn() == Type.GUARD) {
	    		
	    		this.setMessageLabel("GUARD turn");
	    		
	    	}
	    	
	    	else if (board.getTurn() == Type.MUSKETEER) {
	    		this.setMessageLabel("MUSKETEER turn");	
	    	}
    		
    	}
    	
    	else if (this.model.isHumanTurn()) {
	    	if (board.getTurn() == Type.GUARD) {
	    	...
	    	if (board.getTurn() == Type.GUARD) {
	    		this.setMessageLabel("MUSKETEER turn");	
	    		Agent guard = this.model.getGuardAgent();
	    		this.model.move(guard.getMove());
	    	}
	    	
	    	else if (board.getTurn() == Type.MUSKETEER) {
	    		
	    		Agent musketeer = this.model.getMusketeerAgent();
	    		this.setMessageLabel("GUARD turn");
	    		this.model.move(musketeer.getMove());
	    	}	
    	}
    	this.setUndoButton();
    ...
    private void undo() { // TODO
    	...
    	this.boardPanel.setPickedPiece(false);
    	this.model.undoMove();
